...
class CashGame {
    ...
    beforeNewHand(bustedOutArr){        
        ...
        setTimeout(()=>{
            ...
            //erases holeCards and buttonCard and changes circleBorder back to normal
            for (let i = 0; i < this.size; i++){
                ...
                const seatCircle = document.getElementById('seat'+ (i + 1) + 'Div').getElementsByClassName("divCircles")[0]; 
    ...
    calculatePots(){//add later so than you can display pot(s)
        ...
        for (let i = this.betAllInAmts.length - 1; i >= 0; i--){
            ...
            //sets currentPot
            let currentPot = this.tempPots.find(pot => pot.betAmount == currentAmt.betAmount);//if no match then means it's part of pot with bigger amount. Modify code to take this into account            
            if (currentPot == undefined){//find pot with lowest amount greater than
                
                const lessThanIdx = this.tempPots.findIndex(pot=> pot.betAmount < currentAmt.betAmount);
                if (lessThanIdx == -1){
                    currentPot = this.tempPots[this.tempPots.length - 1];
                }
                else{
                    currentPot = this.tempPots[lessThanIdx - 1];
                }
                
                // let tempPotIdx = this.tempPots.length - 1;
                // let foundPot = false;

                // while((!foundPot)||(tempPotIdx != -1)){
                //     if (this.tempPots[tempPotIdx].betAmount > currentAmt.betAmount){
                //         foundPot = true;
                //     }
                //     else{
                //         if (tempPotIdx == 0){
                //             currentPot = this.tempPots[0];
                //         }
                //     }
                //     tempPotIdx--;
                // }
    ...
    endOfRound(){
        ...
        if (this.extraPot){
            ...
            this.tempPots[0].changeBetAmt(0);
        }
    }
        ...
        for (i; i < currentPot.length; i++){ 
            ...
            if(i == (currentPot.length - 1)){//reached end of pot
                ...
                if((secondIdx > 0) || (this.allInCounter == 0) || outerNoAllIns){                 
                    ...
                    if (sidePotNum != 0){
                        document.getElementById('winnerText').textContent = 'Winner(s): ' + 'Side Pot ' + sidePotNum + ': ' + winnerString + endOfString + previousTxt.substring(11);
                        endOfString = ', '
                    }
                    else{
                        document.getElementById('winnerText').textContent = 'Winner(s): ' + 'Main Pot: ' + winnerString + endOfString + previousTxt.substring(11);
                    }
                    sidePotNum--;
                }
